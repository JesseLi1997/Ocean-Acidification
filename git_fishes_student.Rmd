...
library(dplyr)
data2<- select(data1, -c(loc, comment))

# Check spelling in species and treatment but also generate a summary table
library(hunspell)
library(dplyr)
sumtable<- summarise(group_by(data2,species,treatment,size), m.sl=mean(sl), m.activity=mean(activity), se.sl = sd(sl)/sqrt(length(sl)), se.act = sd(activity)/sqrt(length(activity)))
...
library(ggplot2)
data3<-summarise(group_by(data2,species,treatment), m.sl=mean(sl), m.activity=mean(activity), se.sl = sd(sl)/sqrt(length(sl)), se.act = sd(activity)/sqrt(length(activity)))
