...
[My GitHub Repository](https://github.com/JesseLi1997/Ocean-Acidification.git)
...
## **GitHub** is not a file but a storage system. When we lose our files, we can check the history of the file's information, but it's not a file itself. Therefore, **GitHub** is not a back up for files. For example, we can see our codes, but we can't run out codes there.
...
# It will show no code nor results.
```
...
library(readr)
path <- "D:/GitHub/Ocean-Acidification/data/OA_activitydat_20190302_BIOL3207.csv"
...
path <- "D:/GitHub/Ocean-Acidification/output/result.csv"
...
library(readr)
library(tidyr)
data2<- dplyr::select(data1, -c(loc, comment))

# Check spelling in species and treatment but also generate a summary table


sumtable<- dplyr::summarise(dplyr::group_by(data2,species,treatment,size), m.sl=mean(sl), m.activity=mean(activity), se.sl = sd(sl)/sqrt(length(sl)), se.act = sd(activity)/sqrt(length(activity)))

# Use flextable to render the summary table in a tidy format
library(flextable)
ft<- flextable(sumtable)
ft<- theme_vanilla(ft)
ft
```
...
## Green means added lines, and red means deleted lines.
```
...
data3<-dplyr::summarise(dplyr::group_by(data2,species,treatment), m.sl=mean(sl), m.activity=mean(activity), se.sl = sd(sl)/sqrt(length(sl)), se.act = sd(activity)/sqrt(length(activity)))

ggplot(data3, aes(species, m.activity, fill = treatment))+ 
  geom_col(position = 'dodge',width = 0.5)+
  geom_errorbar(aes(x=species,
                ymin = m.activity-se.act,
                ymax = m.activity+se.act), width = 0.05, color = 'black', position = position_dodge(0.5))
...
ggsave("D:/GitHub/Ocean-Acidification/output/figures/figure1.png",last_plot(), dpi = 300)
```
...
```{r}
## The added figure now can be seen in *GitHub Desktop*. 
```
...
```{r, answer6}
## I don't want to track them because I don't want this file to show in my repo.
```
...
data4<- data2[data2$species == 'acantho', ]
data5<- data2[data2$species == 'ambon', ]
data6<- data2[data2$species == 'chromis', ]
data7<- data2[data2$species == 'humbug', ]
data8<- data2[data2$species == 'lemon', ]
data9<- data2[data2$species == 'whitedams', ]
result1<-t.test(activity~treatment, data = data4, paired = FALSE)
result2<-t.test(activity~treatment, data = data5, paired = FALSE)
result3<-t.test(activity~treatment, data = data6, paired = FALSE)
result4<-t.test(activity~treatment, data = data7, paired = FALSE)
result5<-t.test(activity~treatment, data = data8, paired = FALSE)
result6<-t.test(activity~treatment, data = data9, paired = FALSE)
...
df1<-print(as.data.frame(do.call(rbind, result1)))
df2<-print(as.data.frame(do.call(rbind, result2)))
df3<-print(as.data.frame(do.call(rbind, result3)))
df4<-print(as.data.frame(do.call(rbind, result4)))
df5<-print(as.data.frame(do.call(rbind, result5)))
df6<-print(as.data.frame(do.call(rbind, result6)))
l<-data.frame()
for(i in 1:6){
  all<-get(paste0("df",i))
  l<-dplyr::bind_rows(l,all)
}
l
...
Mean activity for `r"acantho"` in the control group was `r"x1=27.3;x1`(s / min) compared to the OA treatment group, which was `r "x2=28.9;x2"` (s / min). The difference between control and OA treatment means was `r "p=0.3;p"` (s / min) (95% CI: `r "x3=-4.8;x3"` to `r "x4=1.6;x4"`).
